...
// ERASE QUOTES
		while (args[j] != NULL) // Loop through all the arguments, so we can CHECK FOR REDIRECTIONS
		{
...
// ">>" APPEND
			if (ft_strcmp(args[j], ">>") == 0 && args[j + 1]) // Check for heredoc
			{
				printf("ENTROU NO >> APPEND\n");

				parsed_commands[i].output_file = ft_strdup(args[1]);
				parsed_commands[i].heredoc = ft_strdup(args[j]); //passing ">>" to heredoc, to signal that it is a append
...
// NO FLAGS
		...
		if (i < num_commands - 1)// If we are not at the last command, create a pipe
		{
...
// "|" PIPES
	...
	parsed_commands[num_commands].cmd = NULL; // Null-terminate the array of commands
